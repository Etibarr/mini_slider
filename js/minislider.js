...
// slider move 
	$('.controll').click(function() {
		...
		if ($(window).width()>'576') {
			if ($id!='controll1'&&$id!='controll'+$lastControll+'') {
				...
				$x=($divCount*($('.carousel_item').width()+$margin));
				$left=(0-($a*$x));
	...
	$('.controll:last-child').click(function() {
			...
			$left=((-$('.carousel_move').width())+($divCount*($('.carousel_item').width()+$margin)));
			$('.carousel_move').css('margin-left', $left);
			return $left;
	});
	...
	$('.right').click(function() {
		if ($left>-($('.carousel_move').width()-2*$('.carousel_item').width()+$margin)) {
			$left=$left-($('.carousel_item').width()+$margin);
			$('.carousel_move').css('margin-left', $left);
			return $left;
		};
	});
...
// carousel resize
	...
	if ($(window).width()<'576') {
			...
			$('.carousel_item').width(($('.carousel_main').width()-($divCount-1)*$margin)/$divCount);
		}
		$('.carousel_move').width(($('.carousel_item').width()+$margin)*$divLength);
